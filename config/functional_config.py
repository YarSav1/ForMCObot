...
async def man_balance(ctx, amount: int):
    DB_GAME.update_one({'id_member': ctx.id},
                       {'$inc': {'balance': amount}})
...
async def list_commands(ctx, admin=None):
    if admin:
        text_commands = \
            ...
            '`!кд` - Проверка и безопасная перезагрузка(без сброса активности). Работают ли все файлы?\n' \
