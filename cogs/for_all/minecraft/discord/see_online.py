...
class Select(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        doc=ONLINE.find_one({'server_name': server, 'name': self.servers[self.servers.index(server)]['ds-minecraft'][1]})
...
async def send_online(author, server, doc, max_lists, now_list):
    ...
    online_days = list(doc['every_day']).reverse()
    text = get_text_online(now_list, foring_start, foring_end, online_days)
    print(text)
    # if now_list <= 0:
    #     left_no
